...
def main():
    ...
    parser.add_argument('--no-serve', action='store_true', help='Only build the UI; do not launch server')
    ...
    if not shutil.which('uvicorn'):
        print('WARNING: uvicorn not found. Install project dependencies first (pip install -e .).', file=sys.stderr)
        sys.exit(1)
    run(['uvicorn', 'yamlguard.server.main:app', '--host', '127.0.0.1', '--port', str(args.port), '--reload'], cwd=ROOT)
